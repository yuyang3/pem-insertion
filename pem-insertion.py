...
def rmb_dedup(fdedup,args):
    ...
    os.system('rm -rf {0}/{1}.sam {0}/{1}.bam'.format(fdedup,args.name))
...
# stitch raw fastq file
def stitch(fdedup,ffastq,args):
    logging.info("Joining the uniq r1 and r2 reads ......")
    # -o prefix -d directory
    exec_log('flash {0}/raw.r1.fq {0}/raw.r2.fq -o {1} -d {2}/flash_out > {2}/flash.log'.format(fdedup,args.name,ffastq))
...
def filter_multiple_adapter(fdedup,args,ffastq):

    if not os.path.exists('{0}/{1}_check.sort.bam'.format(fdedup, args.name)):

        # check input
        if not os.path.exists("{0}/flash_out/{1}.merge.fastq.gz".format(ffastq, args.name)):
            raise ValueError('merged fastq is needed for adapter check alignment.')
        if not os.path.exists('{0}/adapter.fa'.format(fdedup)):
            raise ValueError('adapter/adapter.fa is needed for adapter check alignment.')
        # start bwa mem align

        logging.info("align to check adapter...")
        cmd = "bwa mem -t 8 {0}/adapter -k 5 -L 0 -T 14 {1}/flash_out/{2}.merge.fastq.gz > {0}/{2}_check.sam 2>{0}/bwa_align_check_adapter.log".format(fdedup,ffastq,args.name)
        exec_log(cmd)
        os.system("samtools view -S -b -h {0}/{1}_check.sam > {0}/{1}_check.bam".format(fdedup, args.name))
        os.system("samtools sort -@ 4 {0}/{1}_check.bam > {0}/{1}_check.sort.bam".format(fdedup, args.name))
        os.system("samtools index {0}/{1}_check.sort.bam".format(fdedup, args.name))
        os.system('rm -rf {0}/{1}_check.sam {0}/{1}_check.bam'.format(fdedup, args.name))
    else:
        exec_log("adapter check alignment file exist, jump...")

    # keep record of multiple adapters

    multiple_adapt = open("{0}/{1}_multiple_adapt.txt".format(fdedup,args.name), "w")
    clean_adapt = open("{0}/{1}_clean_adapt.txt".format(fdedup,args.name), "w")
    bam_file = pysam.AlignmentFile("{0}/{1}_check.sort.bam".format(fdedup, args.name), "rb")
    multiple_adapt_list = []
    clean_adapt_list = []
    ...
    os.system("cut -f1 {0}/{1}_barcode_uniq.txt|sort > {0}/tmp1.txt".format(fdedup,args.name))
    os.system("sort {0}/{1}_clean_adapt.txt > {0}/tmp2.txt".format(fdedup,args.name))
    os.system("comm -12 {0}/tmp1.txt {0}/tmp2.txt > {0}/{1}_clean_uniq_adapt.txt".format(fdedup,args.name))
    os.system("rm -rf {0}/tmp1.txt {0}/tmp2.txt".format(fdedup))

    exec_log('seqtk subseq {1}/flash_out/{2}.merge.fastq.gz {0}/{2}_clean_uniq_adapt.txt > {0}/{2}.merge.clean_uniq.fq'.format(fdedup,ffastq,args.name))

def demutiplex_trim_barcode(fdedup,ffastq,args):
    ...
    # de multiplex
    exec_log('fastq-multx -m 0 -x -b -d 0 -B {0}/barcode.txt {2}/{1}.merge.clean_uniq.fq -o {0}/%_demultiplex_merge.fq >{0}/fastq-multx.log 2>&1'.format(ffastq, args.name,fdedup))
...
def map_against_translocation_reference(si,sample,fsample,ffastq,args):
    ...
    if not os.path.exists('{0}/{1}.sort.bam'.format(fsample,translocation_name)):
        ...
        of = open("{0}/{1}_all_information.txt".format(fsample, translocation_name), "w")
        # get perfect_number
        c = 0
        b = 0
        for read in bam_file.fetch():
            ...
            for i in range(1,10):
                ...
                if a == 0 :
                    b+=1
        of.write("all_reads"+"\t"+str(c))
        of.write("perfect_match"+"\t"+str(b))
        of.write("variant_events" + "\t" + str(c-b))
...
# prepare fastq for map
    ...
    if os.path.exists('{}'.format(ffastq)):
        ...
        filter_multiple_adapter(fdedup, args, ffastq)
        demutiplex_trim_barcode(fdedup,ffastq, args)
